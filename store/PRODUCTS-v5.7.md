...
Customer-facing DE/EN/FR/ES/IT/PT-BR/TR names/descriptions are frozen in `apple-iap-metadata.v5.7.json`, `google-one-time-products.v5.7.json` and `google-one-time-products.v5.7.csv`. Apple display names are kept within 30 characters and descriptions within 45 characters. Google titles/descriptions also fit its one-time-product limits.
