# Nyrathen v5.3 — Production scaling and capacity contract
...
- 3 × 100 = **300 active players**: pass, max p99 111.21 ms in the final qualification run.
- 4 × 80 = **320 active players**: passed an earlier safe-capacity run (max p99 157 ms).
- 4 × 100 = **400 active players**: not release-certified; one repeat run reached p99 245.63 ms despite all 400 sessions remaining connected.
- 5 × 100 = **500 active players**: functional connections, but SLO fail from host CPU saturation.
