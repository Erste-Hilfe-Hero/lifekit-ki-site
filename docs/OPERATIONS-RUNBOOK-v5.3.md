# Nyrathen v5.3 — Operations runbook
...
- A hard crash leaves a lock file; v5.3 reclaims it only when it proves the lock belongs to a dead PID on the same host. Unknown or remote locks remain fail-closed.
...
The harness accepts `STRESS_SECONDS` up to one hour per invocation. For release candidates run short CI qualification first, then one-hour worker soaks and multi-node chaos in production-like infrastructure. Track starting/ending RSS, tick p99, error rate, input timeouts and backpressure; a process merely staying alive is not a pass.

## 1k / 2k / 5k / 10k growth

Do not vertically inflate one worker. Use the capacity table in `PRODUCTION-SCALING-v5.3.md`, three workers per logical node maximum, 20% session headroom, and at least one spare worker. `npm run capacity:plan` is the source of truth for required worker/node counts.
