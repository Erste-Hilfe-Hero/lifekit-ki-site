# Nyrathen v5.5 — Production scaling and capacity contract
...
## v5.5 distributed-state staging evidence

The bundled central State Authority is now the supported multi-node state path. A local 1,000-account burst passed at 1,000/1,000, p99 373.47 ms and ~370 opens/s. Railway staging passed 300/300 active SSE players over three physically separate GameServer services with max server p99 35 ms against a 180 ms SLO.

The 1,000-active production contract remains 14 workers across at least five logical nodes with 80 safe players/worker. Current Railway free-plan resource limits do not provide enough services/compute to certify that full topology without a plan change, so the 1,000-active evidence gate remains fail-closed.
